...
describe('To-do list functionality', () => {
  ...
  it('should count the correct number of incomplete tasks in a to-do list', () => {
    ...
    const countIncompleteTasks = todoList.countIncompleteTasks()

    expect(countIncompleteTasks).toBe(1)
  })

  it ('should count the correct number od complete tasks in a to-do list', () => {
    const todoList = new TodoList()
    todoList.addTask(new Task ('1', true))
    todoList.addTask(new Task ('2', false))

    const countCompleteTasks = todoList.countCompleteTasks()

    expect(countCompleteTasks).toBe(1)
  })
